...
const gmailUtils_1 = require("../utils/gmailUtils");
const gmailAgent_1 = require("../agents/gmailAgent");
const googleapis_1 = require("googleapis");
const buffer_1 = require("buffer"); // Needed for base64 decoding
...
const handleRequest = async (req, res, next, taskPromptTemplate) => {
    ...
    try {
        ...
        const tools = (0, gmailUtils_1.createGmailTools)(accessToken);
        ...
        const result = await (0, gmailAgent_1.runGmailAgent)(input, tools);
        res.json(result);
    }
    catch (error) {
 ...
 */
async function ensureDefaultLabelsExist(auth) {
...
function getPlainTextBody(payload) {
    ...
    let htmlPart = null; // Keep track of HTML as a fallback
...
function getAttachmentInfo(payload) {
    ...
    const findAttachmentsRecursive = (currentPart) => {
    ...
    for (let attempt = 1; attempt <= COHERE_MAX_RETRIES; attempt++) {
        try {
            ...
            const response = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Cohere request timed out after ${COHERE_TIMEOUT / 1000} seconds`)), COHERE_TIMEOUT))
            ]);
...
router.post('/summarize-thread', (req, res, next) => {
    ...
    handleRequest(req, res, next, (threadId) => `Use the gmail_get_thread tool to fetch the content of the Gmail thread with ID '${threadId}'. Then, summarize the key points.`);
});
router.post('/extract-tasks', (req, res, next) => {
    handleRequest(req, res, next, (threadId) => `Use the gmail_get_thread tool to fetch the content of the Gmail thread with ID '${threadId}'. Then, extract all action items and tasks mentioned. List them clearly.`);
});
router.post('/follow-up-suggestions', (req, res, next) => {
    handleRequest(req, res, next, (threadId) => `Use the gmail_get_thread tool to fetch the content of the Gmail thread with ID '${threadId}'. Then, review it and suggest potential follow-up actions or emails needed.`);
});
...
router.post('/analyze-emails', async (req, res, next) => {
    ...
    try {
        ...
        let messageListParams = { userId: 'me' };
        if (emailCount) {
        ...
        const cohereClient = new cohere_ai_1.CohereClient({
            token: COHERE_API_KEY
        });
        // Step 5: Fetch details, categorize, and collect results grouped by category
        const categorizedEmails = {}; // Initialize object for grouping
        ...
        const gmailCategoryMap = new Map([
                ...
                if (!payload || !payload.headers) {
                    ...
                    if (!foundCategory || !body) {
                        if (!body && !foundCategory) {
                            ...
                            const categorizationPrompt = `Given the following email details and existing labels, categorize this email into ONE of these categories: ${categories.join(", ")}.
...
**JSON Output:**`;
        ...
        try {
            ...
            if (!analysisData.hasOwnProperty('structured_actions') || !Array.isArray(analysisData.structured_actions)) {
                ...
                analysisData.structured_actions = analysisData.structured_actions.map((action, index) => {
                ...
                }).filter((action) => action !== null); // Remove items that were not valid objects or lacked essential info
            }
            // --- End Deep Validation --- 
...
router.post('/generate-email', async (req, res) => {
    ...
    try {
        // Extract data from request body
        const { subject, body, recipientEmail, tone = 'professional' // Default tone
         } = req.body;
...
router.post('/generate-email-with-revisions', async (req, res) => {
    ...
    try {
        // Extract data from request body
        const { prompt, tone = 'professional', isRevision = false, previousEmails = [], revisionInstructions } = req.body;
