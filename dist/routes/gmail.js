...
const buildRawEmail = (to, from, // Need sender email for headers
subject, body, attachments, cc, bcc) => {
...
router.get('/messages', async (req, res) => {
    ...
    try {
        ...
        const params = { userId: 'me' }; // Use 'me' for the authenticated user
        ...
        if (maxResults && !isNaN(Number(maxResults)))
            params.maxResults = Number(maxResults);
...
function handleGmailApiError(error, res) {
...
router.get('/messages/:messageId', async (req, res) => {
    ...
    try {
        ...
        const response = await gmail.users.messages.get({
            userId: 'me',
            id: messageId,
            format: format // Cast as googleapis types can be strict
        });
        res.json(response.data);
    }
    catch (error) {
...
router.post('/messages/:messageId/modify', async (req, res) => {
    const { messageId } = req.params;
...
router.post('/messages/:messageId/snooze', async (req, res) => {
    const { messageId } = req.params;
